...
def get_gdp(country_code, date):
    ...
    url = f"http://api.worldbank.org/v2/country/{country_code}/indicator/NY.GDP.MKTP.CD?date={date}&format=json"
    resp = requests.get(url)    #response object
    data = resp.text    #jason-formatted str
    data_list = json.loads(data)
    print(data_list)
    return data_list[1][0]["value"]
...
def get_data(country_code, first_year, second_year):
    ...
    url = f"http://api.worldbank.org/v2/country/{country_code}/indicator/NY.GDP.MKTP.CD?date={first_year}:{second_year}&format=json"
    resp = requests.get(url)    #response object
    data = resp.text    #jason-formatted str
    data_list = json.loads(data)
    print(data_list)
...
def gdp_growth(country_code, first_year, second_year):
    ...
    data_list = get_data(country_code, first_year, second_year)
    val1 = data_list[1][0]["value"]
    val2 = data_list[1][1]["value"]
    return (val2, val1)
...
def main():
    ...
    first_year = 2020
    second_year = 2019
    get_data(country, first_year, second_year)
